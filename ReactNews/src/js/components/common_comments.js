...
class CommonComments extends React.Component {
	...
	handleSubmit(e) {
		...
		if (!localStorage.userid) {
			notification['success']({message: 'ReactNews提醒', description: '请先登录'});
			return
		}
		var myFetchOptions = {
			method: 'GET'
		...
		if (formdata.remark ===undefined) {
				notification['success']({message: 'ReactNews提醒', description: '评论不能为空'});
				return
			}
		//添加评论接口
				...
				<Row>
					<Col span={24}>
						...
						<Form onSubmit={this.handleSubmit.bind(this)}>
							<FormItem label="您的评论">
								{getFieldDecorator('remark')(<Input type="textarea" placeholder="您的评论"/>)}
							</FormItem>
